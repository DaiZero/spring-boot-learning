...
public class IpUtil {
    ...
    public static String getDomainNameExitPort(HttpServletRequest request) {
        ...
        String domainName =
                url.delete(url.length() - request.getRequestURI().length(), url.length()).append(FOLDER_SEPARATOR).toString();
        if (domainName.contains(COLON)) {
            int i = domainName.length() - domainName.indexOf(COLON);
            domainName = domainName.substring(0, (i - 2));
        }
        String tempDomainName = domainName.substring(domainName.length() - 1);
        if (!tempDomainName.contains(FOLDER_SEPARATOR)) {
            domainName = domainName + FOLDER_SEPARATOR;
        }
        return domainName;
    }
    ...
    public static String getDomainNam(HttpServletRequest request) {
        //获取域名
        return "http://" + request.getLocalAddr() + COLON + request.getLocalPort();
    }
    ...
    public static String getIpAddr(HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(ip) || ip.length() == 0 || UNKNOWN_STRING.equalsIgnoreCase(ip)) {
